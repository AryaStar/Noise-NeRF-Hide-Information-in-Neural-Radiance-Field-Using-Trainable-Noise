...
def config_parser():
    ...
    parser.add_argument("--N_thresh",   type=int, default=6144, 
                        help='N_thresh')
    parser.add_argument("--Loop",   type=int, default=300, 
                    help='Loop')
    return parser
...
# adv_img
im = asarray(Image.open("adv/dogs.png").resize((H,W)))
...
# save_image(torch.permute(my_adv_img, (2, 0, 1)), 'zorigin.png')

save_image(torch.permute(my_adv_img, (2, 0, 1)), 'experiment/'+expname+'/'+'origin.png')
...
# for num, i in enumerate(range(0, W*H, N_thresh)):
    ...
    viewdirs = viewdirs_all.reshape((W,H,-1))[select_coords[:, 1], select_coords[:, 0]] # (N_rand, 3)
    z_vals = z_vals_all.reshape((W,H,-1))[select_coords[:, 1], select_coords[:, 0]] # (N_rand, 64)
    
    # target and adv img
    # target_img_s = target_img[select_coords[:, 1], select_coords[:, 0]]  # (N_rand, 3)
    adv_img_s = adv_img[select_coords[:, 1], select_coords[:, 0]] # (N_rand, 3)
    
    # encoded_points
    encoded_points = encoded_points_all.reshape((W,H,64,90))[select_coords[:, 1], select_coords[:, 0]].reshape((-1,64,90))
    ...
    encoded_points_all.reshape((W,H,64,90))[select_coords[:, 1], select_coords[:, 0]] = adv_points
...
save_image(torch.permute(my_adv_img, (2, 0, 1)),'experiment/'+expname+'/'+expname+'.png')
file = open("experiment/"+expname+"/record/"+"ssim.txt", "w")
for cont, item in enumerate(loss_list):
    file.write(ssim(my_adv_img.cpu().numpy(), adv_img.cpu().numpy(), multichannel=True))
file.close()
